...
var pollIntervalMin = 1;  // 5 minutes
var pollIntervalMax = 2;  // 1 hour
...
function getOwnCloudNewsUrl() {
  //return "http://wolk.rambo-mes.net/index.php/apps/news/";
    ...
    xhr.onreadystatechange = function() {
      ...
      if (xhr.response) {
        ...
        if (obj.feeds) {
          ...
          console.error(chrome.i18n.getMessage("owncloudnews_node_error"));
        }
        return;
      }
...
function onNavigate(details) {
  if (details.url && isOwnCloudNewsUrl(details.url)) {
    console.log('Recognized ownCloud navigation to: ' + details.url + '.' +
                'Refreshing count...');
    startRequest({scheduleRequest:false, showLoadingAnimation:false});
  }
}
